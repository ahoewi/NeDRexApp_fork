...
/*		if (network.getRow(selectedNodes.get(0)).get("type", String.class).equals(NodeType.Gene.toString())) {
		...
		if (!ggType) {
			proteinDrugsMap = this.apiUtils.getDrugsTargetingProteins(network, selectedNodes, targeting_drugs);
		}
		else if (ggType) {
			proteinDrugsMap = this.apiUtils.getDrugsTargetingGenes(network, selectedNodes, targeting_drugs);
		}
		logger.info("Here's the set of targeting drugs: " + targeting_drugs);
		if(only_approved_drugs) {
			List<String> approved_drugs = this.apiUtils.getApprovedDrugsList(targeting_drugs);
